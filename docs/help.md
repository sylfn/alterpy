...
Альтер по умолчанию общается на русском языке, но у некоторых команд есть поддержка английского языка. Это --- управляющая команда. Узнать свой язык можно с помощью `?lang`, сбросить через `-lang`, выставить через `+lang [language]`. Поддерживаемые строки языка: `ru`, `en` (защиты от других строк нет, в таком случае до добавления поддержки такого языка будет использоваться русский).
